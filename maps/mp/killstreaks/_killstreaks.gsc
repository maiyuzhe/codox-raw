...
onPlayerSpawned()
{
	...
	for ( ;; )
	{
		...
		self thread drawKillstreakCounter();
...
shuffleKillStreaksFILO( streakName )
{
	...
	for (i = 0; i <  3; i++ )
	{
		if (streakName == self getPlayerData( "killstreaks", i ) ) 
		{
			...
			{
				self _setActionSlot( ( i +1 ), "" );
			}
		}
		if ( i == 2 && streakName != self getPlayerData( "killstreaks", i) )
		{
...
giveKillstreak( streakName, isEarned, awardXp, owner )
{
	...
	self giveKillstreakWeapon( weapon, isEarned );
...
giveKillstreakWeapon( weapon, isEarned )
{
...
drawKillstreakCounter()
{
	...
	for ( i = 0; i < 3; i++)
	{
		if ( self getPlayerData("killstreaks", i) == "none" )
		{
			switch(i)
			{
				case 0:
					self.barHeight = self.streakBar["height_one"];
					break;
				case 1:
					self.barHeight = self.streakBar["height_three"];
					break;
				case 2:
					self.barHeight = self.streakBar["height_two"];
					break;
			}
		}
	}
	...
	{
		self.bar[i] = NewClientHudElem( self );
		self.bar[i].x = 45;
		self.bar[i].y = -30 - (i*6);
		self.bar[i].alignX = "center";
	...
	for( ;; )
	{
		...
		{
			for (i = 0; i < self.streakBar["height_one"]; i++)
			{
				self.bar[i].alpha = 1;
			}
			if (self.pers["cur_kill_streak"] < self.streakBar["height_one"])
			{
				for ( i = self.streakBar["height_one"]; i < self.barHeight; i++)
				{
					self.bar[i].alpha = 0;
				}
			}
			...
			if (self.pers["cur_kill_streak"] < self.streakBar["height_two"])
			{
				for ( i = self.streakBar["height_two"]; i < self.barHeight; i++)
				{
					self.bar[i].alpha = 0;
				}
			}
