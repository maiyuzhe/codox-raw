...
init()
{
	...
	*/
	...
	//stalker
	level.perkSetFuncs["specialty_localjammer"] = ::setStalker;
	level.perkUnsetFuncs["specialty_localjammer"] = ::unsetStalker;
	...
	level.perkSetFuncs["specialty_onemanarmy"] = ::setLightWeight;
	level.perkUnsetFuncs["specialty_onemanarmy"] = ::unsetLightWeight;	
...
cac_modified_damage( victim, attacker, damage, meansofdeath, weapon, impactPoint, impactDir, hitLoc )
{
	...
	else if ( isExplosiveDamage( meansOfDeath ) )
	{
		...
		//replacing danger close with blastshield effect
		// else if ( victim _hasPerk( "_specialty_blastshield" ) )
		// 	damageAdd -= damage*(1-level.blastShieldMod);
		else if ( victim _hasPerk( "specialty_explosivedamage" ) )
			damageAdd -= damage*(1-level.blastShieldMod);
		//old 
